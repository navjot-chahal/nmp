...
/*
 * Authentication APIs (DocScan)
 */

// Initialize authentication process with a document scan
func (s *LoginID) AuthenticateDocScanInit(clientID string, userID string, credentialUUID string) (*AuthenticateDocScanInitResponse, error) {
	h := map[string]string{
		"X-Client-ID": s.ClientID,
	}
	if s.PrivateKey != nil {
		jwt, err := s.GenerateServiceToken("auth.login", &token.ServiceTokenOptions{UserID: userID})
		if err != nil {
			return nil, err
		}
		h["Authorization"] = fmt.Sprintf("Bearer %s", jwt)
	...
	if credentialUUID != "" {
		p["options"] = map[string]string{"credential_uuid": credentialUUID}
	}
	d, err := json.Marshal(p)
	if err != nil {
		return nil, err
	...
	res, err := s.Http.Post(fmt.Sprintf("%s/authenticate/authid/init", s.BaseURL), bytes.NewBuffer(d), h)
	if err != nil {
		return nil, err
	}

	var r *AuthenticateDocScanInitResponse
	if err := s.Http.DecodeResponse(res, &r); err != nil {
		return nil, err
	}
	return r, nil
...
func (s *LoginID) AuthenticateDocScanComplete(clientID string, userID string, username string, credentialUUID string) (*AuthenticationResponse, error) {
	h := map[string]string{
		"X-Client-ID": s.ClientID,
	}
	...
	res, err := s.Http.Post(fmt.Sprintf("%s/authenticate/authid/complete", s.BaseURL), bytes.NewBuffer(d), h)
	if err != nil {
		return nil, err
	}
